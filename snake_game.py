...
# if game is not running, checks buttons to start game...otherwise,
# button1 turns user left (relative to current direction), and button2
# turns user right (relative to current direction)
def check_buttons():
    global buttonCounter
    global direction
    global running
...
def game_over():
    ...
    if running == True:
        ...
        display.text('Game Over: ' + str(length),15,30,0)
        display.show()
        reset_variables()
        time.sleep(2)
